...
function ttfb_toolkit_font_awesome_font_awesome_scripts() {
    ...
    if( $fa_active ){
        ...
        if( $brands == 1 && $librairy != 'all' ){
            wp_enqueue_script( 'fontawesome-brands', plugin_dir_url( __FILE__ ) . '../vendor/fontawesome/brands.js', array(), '5.1.0', true );
        }
    
        if( $librairy == 'solid' ){
            wp_enqueue_script( 'fontawesome-solid', plugin_dir_url( __FILE__ ) . '../vendor/fontawesome/solid.js', array(), '5.1.0', true );
        }elseif( $librairy == 'regular' ){
            wp_enqueue_script( 'fontawesome-regular', plugin_dir_url( __FILE__ ) . '../vendor/fontawesome/regular.js', array(), '5.1.0', true );
        }elseif( $librairy == 'all' ){
            wp_enqueue_script( 'fontawesome-all', plugin_dir_url( __FILE__ ) . '../vendor/fontawesome/all.js', array(), '5.1.0', true );
        }
    
        if( $shims == 1 ){
            wp_enqueue_script( 'fontawesome-v4-shim', plugin_dir_url( __FILE__ ) . '../vendor/fontawesome/v4-shims.js', array(), '5.1.0', true );
        }
        
        if( $librairy != 'all' ){
            wp_enqueue_script( 'fontawesome-core', plugin_dir_url( __FILE__ ) . '../vendor/fontawesome/fontawesome.js', array(), '5.1.0', true );
        }
    }
